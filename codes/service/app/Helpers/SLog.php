...
class SLog
{
     ...
     * @var bool
     */
    public static $isOpenMysqlLog = true;
    ...
    protected static $css=array(
        ...
        'log'=>'color: #333;background-color: #fff;',
        'info'=>'color: #fff;background-color: #5bc0de;',
        'warn'=>'color: #fff;background-color: #f0ad4e;border-color: #eea236;',
        'error'=>'color: #fff;background-color: #d9534f;border-color: #d43f3a;',
        'trace'=>'color: #fff;background-color: #337ab7;border-color: #2e6da4;'
    );
    ...
    //记录日志
    public function record($type, $msg='', $name= '')
    {
        /*
         * 此处不做检查 把数据写入$logs中
         * if(!self::check())
        ...
        $css = '';
        if (strstr($name, 'color') !== false || strstr($name, 'background') !== false ) {
            $css = $name;
            $name = '';
        }
    ...
    protected static function writeLog()
    {
        ...
        foreach (self::$logs as $key=>$value) {
            ...
            $msg = is_string($value['msg']) ? json_encode($value['msg']) : $value['msg'];
        ...
        $logType = empty($logType) ? 'info' : $logType;
    }
